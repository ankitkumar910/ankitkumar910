...
function Bottombar() {
    ...
    return (
        <nav
            className={`fixed sm:hidden py-1.5  bottom-0 left-0 right-0 z-50 transition-all duration-300 
        ...
        bg-white  shadow-xl shadow-amber-600 dark:bg-[#1a1814]/90 backdrop-blur-md  border-b border-amber-200/50 dark:border-amber-900/30"
          : "bg-transparent"
      `}>
            <div className="  h-16  justify-around">
                {/* Desktop nav */}
                <ul className="flex justify-evenly">
                    {navLinks.map((link) => (
                        <li key={link.id}>
                            <a
                                href={`#${link.id}`}
                                className={`ubuntu-regular text-sm px-5 py-1 my-2 flex flex-col 
                                    
                                    
                                    justify-start items-center
                                    rounded-3xl  shadow-2xl transition-all duration-200  ${activeSection === link.id
                                        ? "bg-amber-600 text-white"
                                        : "text-gray-700 dark:text-gray-300  "
                                    } `}>
                                {link.label == "Home" && <Home fontSize="medium" />}
